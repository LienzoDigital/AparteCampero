  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditing) {
      updateTeam(currentTeam.id, {
        ...
        category: currentTeam.number,
      });
      setIsEditing(false);
    } else {
      addTeam({ name: currentTeam.name, category: currentTeam.number });
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCurrentTeam({ ...currentTeam, number: e.target.value })
                  }
        ...
        <div className="lg:col-span-2">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {teams.map((team) => (
              <Card
                key={team.id}
                className="group hover:border-primary-200 dark:hover:border-primary-800 transition-all"
              >
                <div className="flex justify-between items-start">
                  <div>
                    <div className="text-xs font-bold text-secondary-400 uppercase mb-1">
